...
export default function Register() {
  ...
  return (
    <div className="max-w-sm mx-auto px-mobile py-16">
      <h1 className="font-headline-lg text-headline-lg text-center text-gray-800 mb-8">{t('register')}</h1>
      {error && <div className="bg-red-50 border border-red-100 text-red-500 rounded-xl p-3 mb-4 text-caption">{error}</div>}
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block font-caption text-caption text-gray-500 mb-1">{t('name')}</label>
          <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white" />
        </div>
        <div>
        ...
        <div>
          <label className="block font-caption text-caption text-gray-500 mb-1">{t('confirm_password')}</label>
          <input type="password" required value={confirm} onChange={e => setConfirm(e.target.value)} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 font-body-md text-body-md focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white" />
        </div>
      ...
      <p className="text-center font-caption text-caption text-gray-400 mt-5">{t('have_account')} <Link to="/login" className="text-emerald-600 hover:underline">{t('login')}</Link></p>
    </div>
  );
}
