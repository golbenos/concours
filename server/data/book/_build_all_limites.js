...
const base = path.join(root, 'data', 'book');
...
const limitesCourse = {
  course: [{
    ...
    items: [
      "Méthodes autorisées (2BAC SM) : factorisation, conjugué quantités conjuguées, théorème des gendarmes, limites usuelles $\\sin x/x$, $(e^x-1)/x$, $\\ln(1+x)/x$, $\\arctan x/x$, croissances comparées, règle de l'Hôpital pour les formes $0/0$ ou $\\infty/\\infty$.",
      "**Rappels utiles** : $\\lim_{x\\to0}\\frac{\\sin x}{x}=1$, $\\lim_{x\\to0}\\frac{e^x-1}{x}=1$, $\\lim_{x\\to0}\\frac{\\ln(1+x)}{x}=1$, $\\lim_{x\\to0}\\frac{\\arctan x}{x}=1$, $\\lim_{x\\to+\\infty}\\frac{\\ln x}{x}=0$, $\\lim_{x\\to+\\infty}x^n e^{-x}=0$, $\\lim_{x\\to-\\infty}x^n e^{x}=0$.",
      "**Règle de L'Hôpital** : Pour $\\frac{0}{0}$ ou $\\frac{\\infty}{\\infty}$, si $\\lim\\frac{f'(x)}{g'(x)}$ existe, alors $\\lim\\frac{f(x)}{g(x)} = \\lim\\frac{f'(x)}{g'(x)}$.",
      "**Techniques classiques** : multiplier par le conjugué pour les racines, utiliser $\\sin x\\sim x$ et $\\cos x-1\\sim -x^2/2$ pour les limites en $0$, factoriser le terme dominant pour les limites en $\\infty$.",
      "**Forme exponentielle** : Pour $f(x)^{g(x)}$, écrire $\\ln y = g(x)\\ln f(x)$ et étudier la limite."
    ]
  }]
};
...
fs.writeFileSync(path.join(base, 'limites-qcm.json'), JSON.stringify(content, null, 2) + '\n', 'utf-8');
